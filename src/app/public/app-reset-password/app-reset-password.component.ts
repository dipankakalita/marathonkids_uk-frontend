...
@Component({
  selector: 'app-reset-password-app',
  templateUrl: './app-reset-password.component.html',
  styleUrls: ['../login/login.component.scss', './app-reset-password.component.scss']
})
export class AppResetPasswordComponent implements OnInit {
  ...
  onSubmit(event) {
    ...
    this.authService.appResetPassword(this.tokenId, this.resetForm.value).subscribe((result) => {
      this.loading = false;
