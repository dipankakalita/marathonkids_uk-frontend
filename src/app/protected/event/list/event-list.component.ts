...
import { HttpCommonService, HttpService,AuthService } from 'src/app/core/services';
...
export class EventListComponent implements OnInit, OnDestroy {
    ...
    syncEventData() {
		...
		if(this.accountType =='Parks'){
			...
			this.htcService.post('event/saveEventData', {
				verified: false,
    ...
    loadEvents(type = null) {
		...
		var API_URL = 'event/search';
		if(this.accountType =='Parks'){
			var API_URL = 'event/geteventsListforpark';
			var API_URL_ActiveEvent = 'event/getactiveeventsListforpark';

			options.where.runnercount = { $ne : '0' };
			//options.where.runnercount = { $exist : true};
			optionsactive.where.runnercount = { $ne : '0' };
		}
		
		this.htcService.post(API_URL_ActiveEvent, optionsactive).subscribe(async(result) => {
			this.totalactive = result.data;
		});
    ...
    onAdd(): void {
        const dialogRef = this.matDialog.open(EventFormDlgComponent, {
            width: '500px',
            data: {
                action: FormType.NEW
            }
    ...
    onDelete(id): void {
        ...
        confirmDialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.loading = true;
                this.htcService.delete('event', id).subscribe((res) => {
                    this.loading = false;
                    this.loadEvents();
                }, (err) => {
                    this.loading = false;
