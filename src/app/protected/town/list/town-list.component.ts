...
export class TownListComponent implements OnInit, OnDestroy {
    ...
    loadTowns(type = null) {
        ...
        this.htcService.post('town/search', options).subscribe((result) => {
    ...
    onAdd(): void {
        const dialogRef = this.matDialog.open(TownFormDlgComponent, {
            width: '500px',
            data: {
                action: FormType.NEW
            }
    ...
    onDelete(id): void {
        ...
        confirmDialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.loading = true;
                this.htcService.delete('town', id).subscribe((res) => {
                    this.loading = false;
                    this.loadTowns();
                }, (err) => {
                    this.loading = false;
